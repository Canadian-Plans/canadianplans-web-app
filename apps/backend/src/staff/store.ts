...
export class DatabaseStaffStore implements StaffStore {
  async bootstrapStaff(input: BootstrapStaffInput): Promise<StaffWorkspace[]> {
    return withActorTx(input.actorId, async (tx) => {
      await tx.execute(
  ...
  async loadStaffAccess(
    ...
    return withTenantTx({ actorId, workspaceId }, async (tx) => {
  ...
  async inviteStaff(input: InviteStaffInput): Promise<string> {
    return withTenantTx({ actorId: input.actorId, workspaceId: input.workspaceId }, async (tx) => {
      ...
      const roleRows = await tx
        ...
        .from(roles)
        .where(and(eq(roles.workspaceId, input.workspaceId), inArray(roles.name, input.roleNames)));
